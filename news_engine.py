...
# Brought back deliberately narrow in scope: ONLY used to score impact severity
# (the color dot). Symbol tagging, relevance filtering, and bullet extraction all
# stay keyword-based/extractive as before - none of that changes. If this key is
# missing or the call fails, severity silently falls back to the existing
# keyword-based heuristic rather than blocking anything.
...
# --- AI severity scoring (narrow scope: ONLY the color, nothing else) ---
GEMINI_MODEL = "gemini-2.5-flash-lite"
GEMINI_RPM_LIMIT = 12  # stay under the free-tier per-minute ceiling with margin
_gemini_call_times = []
...
def _gemini_rate_limit_wait():
    ...
    if len(_gemini_call_times) >= GEMINI_RPM_LIMIT:
        ...
        if wait_for > 0:
            print(f"Gemini RPM budget reached - pausing {wait_for:.1f}s.")
            time.sleep(wait_for)
    _gemini_call_times.append(time.time())


def ai_score_impact(headline, summary, market_symbol):
    """Returns one of 🔴🟠🟡⚪, or None on any failure - caller keeps the
    keyword-based color as a fallback, so a Gemini outage never blocks a send,
    it just means that one alert uses the slightly less precise fallback color."""
    if not GEMINI_KEY:
        return None

    prompt = f"""You are scoring the market-impact severity of a single news headline for
a day trader watching {market_symbol}. Choose exactly ONE severity level:

HIGH - an active, ongoing shock or major scheduled catalyst happening NOW: a surprise
policy move, an active escalation/attack, an actual rate decision, a surprise NFP/CPI print,
a large-cap M&A deal (>$1B) actively being announced.

MEDIUM - notable and worth knowing but not urgent: de-escalation/ceasefire news, a
routine scheduled data point, a mid-size M&A deal, meaningful but expected developments.

LOW - on-topic but minor: routine company news, small-cap M&A, a preview/roundup
piece mentioning several things, minor data revisions.

NEUTRAL - not clearly relevant to active trading decisions right now, or too minor to matter.

Respond with ONLY one word: HIGH, MEDIUM, LOW, or NEUTRAL. Nothing else.
...
"""
    ...
    payload = {
        ...
        "generationConfig": {
            "temperature": 0.1,
            "maxOutputTokens": 10,
            "thinkingConfig": {"thinkingBudget": 0},  # this is a one-word classification,
                                                        # not a reasoning task - thinking here
                                                        # only adds latency and empty-response risk
        }
    }
    ...
    for attempt in range(2):
        try:
            ...
            if res.status_code == 200:
                ...
                mapping = {"HIGH": "🔴", "MEDIUM": "🟠", "LOW": "🟡", "NEUTRAL": "⚪"}
                for key, emoji in mapping.items():
                    if key in text:
                        return emoji
                print(f"Gemini severity: unrecognized response {text!r}, using keyword fallback.")
                return None
            elif res.status_code == 429 and attempt == 0:
                time.sleep(10)
                continue
            else:
                print(f"Gemini severity error {res.status_code} - using keyword fallback.")
                return None
        except Exception as e:
            if attempt == 0:
                continue
            print(f"Gemini severity request failed ({e}) - using keyword fallback.")
            return None
    return None
...
def fetch_rss_articles(feed_url, source_name):
    ...
    try:
        ...
        # Strip characters that are illegal in XML 1.0 but occasionally sneak into
        # real-world feeds (stray control characters from copy-pasted text, encoding
        # quirks). This is the actual cause of most "not well-formed (invalid token)"
        # errors - the feed itself is fine, just has a few bad bytes in it.
        import re
...
def process_live_news(state, now_ts, browser=None):
    ...
    try:
        ...
        for item in articles:
            ...
            # AI severity scoring, narrow scope: only overrides the color if it
            # succeeds. Keyword-based impact_emoji from classify_article above
            # stands as the fallback if this returns None for any reason.
            ai_impact = ai_score_impact(headline, summary, classification.get("market_symbol", "USD"))
            if ai_impact:
                classification["impact_emoji"] = ai_impact
